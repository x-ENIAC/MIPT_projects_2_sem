...
Statuses_type draw_points(Mandelbrot_params* parameters) {
	...
	for(int y = 0; y < parameters->height_screen; ++y) {
		...
		float start_x = ((- 400.f) * dx + ROI_X + xC ) * scale, start_y = ( ((float)y - 300.f) * dy + ROI_Y + yC ) * scale;

		for(int x = 0; x < parameters->wigth_screen; ++x, start_x += dx) {
			if(is_escape_pressed())
				return ESCAPE_PRESSED;
...
RGBQUAD get_colour(Coordinates* coordinates, Mandelbrot_params* parameters) {
	float transparency = sqrtf(sqrtf((float)coordinates->step / (float)parameters->max_steps));
